...
def _extract_system_warnings(replies, chat_id):
    """Remove system_warning and stop_signal replies and send warnings as Lark cards.

    Returns the remaining (non-system) replies.  stop_signal messages are
    generated by the worker when the card Stop button is clicked — they are
    only meaningful to the agent-mode monitor and should be silently dropped
    in normal handoff mode.
    """
    _system_types = ("system_warning", "stop_signal")
    warnings = [r for r in replies if r.get("msg_type") == "system_warning"]
    remaining = [r for r in replies if r.get("msg_type") not in _system_types]
    for w in warnings:
